# Delivery Baseline Performance Test Protocol

## Goal

Run 3 baseline performance tests against the current workflow.

All data stays the same across runs except:

- `companyName`
- `email`

Detailed behavior, tool activity, and per-step timing should be audited in the session log, not in this document.

## Test Data

| Test ID | Company Name | Email |
|---|---|---|
| BL-01 | BMQAP01 | bmqap01@example.com |
| BL-02 | BMQAP02 | bmqap02@example.com |
| BL-03 | BMQAP03 | bmqap03@example.com |
Use the same workflow inputs for all 3 runs:

- Start intent: `create client`
- Lead Type: `Landing Tree`
- Delivery schedule: `24/7`
- Delivery Method: `Webhook`
- Webhook URL: `https://example.com/baseline-webhook`
- Field mappings: `I'll provide instructions`
- Content type: `JSON`
- Posting instructions: use the JSON below exactly
- Connection test: `Skip`
- Lead price: `$200`
- Exclusive: `Yes`
- Use Order System: `Yes`
- Target states: `California, Texas`
- Additional criteria: `Skip`
- Final action: `Keep Inactive`
...
```json
{
  ...
  "mobile_phone": "2065559876",
  "second_phone": "2065554321",
  "dob": "1988-07-14",
  "loan_amount": 250000,
  "balance": 185000,
  "loan_type": ["Refinance"],
  "estimated_value": 420000,
  "lender_date": "2026-03-31T11:00:00Z",
  "request_date": "2026-03-31",
  "vendor_id": "EB1234",
  "ad_source": "Google Ads",
  "sub_source": "Andres Cafe",
  "sub_id": "SUB-ABC-789",
...
3. Run the full setup using the fixed inputs above.
4. Record the total processing time for the full run.
5. Audit all detailed timing, tool execution, outputs, and reasoning from the session log.

Exclude human response time from the total processing time if your measurement method allows that.
